...
**2026-09-14 改訂**: QPU候補をeu-north-1のIQM Garnet / Emerald、AQT IBEX-Q1に固定する。
未知のデバイスを通し得る拒否リスト方式を廃止し、3機以外を拒否するIAMガードレールへ移行する。
費用のハードストップにはBraket Spending Limitを追加する。詳細はADR-0003。
...
- **IAMでショット数は制限できない**: ADR-0003でBraket Spending Limitを追加し、
  QPU費用をサービス側から止める。AWS Budgetsはその他の費用を含む通知として残す
- **拒否リスト方式は移行対象**: ADR-0003に従い、候補3機以外を拒否するガードレールへ改訂し、
  IAM Policy Simulatorで効果を実測してから適用する
