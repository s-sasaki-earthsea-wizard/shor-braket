...
`make report` は同じディレクトリに `analysis.json`（同時分布・指標・判定）と `report.md` を書く。
`execution.class = qpu`。**λ の合否判定はサポート質量由来の推定量**で行い、TVD 由来の λ は標本床と
並べて表示するだけで判定には使わない。位数復元率は一様乱数の基準値と並べて最後に置く。
